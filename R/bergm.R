...
#' A formula specifying one or more constraints on the support of the distribution 
#' of the networks being modeled, using syntax similar to the formula argument, on 
#' the right-hand side. Multiple constraints may be given, separated by “+” and “-” 
#' operators. (See \code{\link[ergm]{ergm}} constraints for the explanation of their 
#' semantics.) Together with the model terms in the formula and the reference measure, 
#' the constraints define the distribution of networks being modeled.
#'
#' @param thin count; 
...
bergm <- function (formula, prior.mean = NULL, prior.sigma = NULL, burn.in = 100, 
          ...
          V.proposal = 0.0025, startVals = NULL, offset.coef = NULL, constraints = NULL, thin = 1,
          ...) 
{
  ...
  if (!is.null(offset.coef)) {
    if (length(control$init[model$etamap$offsettheta]) != 
      ...
      stop("Invalid offset parameter vector offset.coef: ", 
           "wrong number of parameters: expected ", length(control$init[model$etamap$offsettheta]), 
  ...
  out = list(Time = runtime, formula = formula, specs = model$coef.names, 
             dim = dim, Theta = FF, AR = AR, ess = ess, constraints = constraints)
  class(out) <- "bergm"
  return(out)
}
