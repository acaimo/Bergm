...
#' 
#' @param ... additional arguments, to be passed to the ergm function. See \code{\link[ergm]{ergm}} and \code{\link[Bergm]{ergmAPL}}.
#' 
...
evidencePP <- function(formula,
  ...
  for (j in 1:numtemp) {
    ...
    if (j == 1) {
      grad.logpp.list[[j]] <- t(apply(data.frame( pplist[[j]] ), 1,
                                      function(x) -solve(prior.sigma, (x - prior.mean))))
      
      logPL.list[[j]]    <- apply(pplist[[j]] , 1,
                                  function(x) dmvnorm(x, prior.mean, prior.sigma, log = TRUE))
  ...
  ppml <- function(cv.ell.D2, 
    ...
    for (i in 1:(N - 1)) {
      ...
      cv.res.D2.mts <- cv.res.D2.mts + wts * ((cv.ell.D2[i + 1] + cv.ell.D2[i]) / 2.0 ) - 
  ...
  out <- list(formula      = formula,
              Theta        = pplist[[numtemp]], # mcmc object; MCMC draws at t_j = 1 
              AR_temp      = acceptances,
              AR           = acceptances[numtemp],
              ess          = ess,          # Effective sample size for t_j = 1
